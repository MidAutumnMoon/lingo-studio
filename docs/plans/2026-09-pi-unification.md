...
| Fact | Value |
|---|---|
| Pinned pi versions | riding the ladder to 0.87.1 — currently `@earendil-works/pi-ai` 0.82.1, `@earendil-works/pi-coding-agent` 0.82.1; the pi line (including transitive `pi-agent-core`/`pi-ai`) is forced to the root pin via `pnpm-workspace.yaml` overrides (see upgrade-notes §8) |
