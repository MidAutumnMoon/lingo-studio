 ...
 * (`PI_PLACEHOLDER_API_KEY`); the real Cherry key is injected separately as an
 * in-memory runtime override on the isolated `ModelRuntime` so raw keys never
 * enter pi's config-value interpolation or any persisted pi file.
 *
 * The connection ALSO registers the same provider directly on the isolated
 * `ModelRuntime` before session creation — pi resolves the session model before
 ...
 * injection point; both target the same runtime, and `registerProvider` is an
 * idempotent upsert.
 */
