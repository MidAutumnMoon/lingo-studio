...
- **Write conventional commits**: commit small, focused changes as `type(scope): summary`. The scope names a specific kebab-case module (`fix(lifecycle):`, `chore(deps):`), never something generic like `main`. Deviations in history don't set precedent.
- **Land on `main`**: all development lands on the `main` bookmark via `jj git push`. No side branches or per-change bookmarks.
